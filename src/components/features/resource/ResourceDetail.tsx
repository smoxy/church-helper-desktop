...
import { OptimizedActiveNotice } from './OptimizedActiveNotice';
...
export function ResourceDetail({ resource, onClose }: ResourceDetailProps) {
    ...
    // Collaudo bug 1: when the preference is ALREADY on, the callout above
    // never rendered and the user got no feedback that this resource is
    // being downloaded lighter. Same slot, positive/success variant instead
    // of an invite — mutually exclusive with showOptimizedCallout since both
    // are gated on preferOptimized/!preferOptimized.
    const showOptimizedActiveNotice = preferOptimized && hasOptimizedVariant;
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 backdrop-blur-xs" onClick={onClose}>
            <div role="dialog" aria-modal="true" aria-label={resource.title} className="bg-card text-card-foreground rounded-xl shadow-2xl max-w-2xl w-full p-6 animate-in fade-in zoom-in duration-200" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-start mb-6">
                    ...
                    <button ref={closeButtonRef} onClick={onClose} aria-label={t('resourceDetail.close')} className="text-muted-foreground hover:text-foreground">
                        <X className="h-6 w-6" />
                    </button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="flex flex-col gap-4 mt-6">
                            <div className="w-full">
                                <div className="flex justify-between items-center mb-2">
                                    <h3 className="text-lg font-semibold">{t('resourceDetail.download')}</h3>
                                ...
                                <div className="flex flex-col gap-3">
                                    ...
                                    {showOptimizedActiveNotice && (
                                        <OptimizedActiveNotice savings={savings} />
                                    )}
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <button
                                            ...
                                            `}
                                            title={isCorrupted ? t('resourceDetail.retryHint') :
                                                isDownloaded ? t('resourceDetail.openInFolder') :
                                                    isPending ? (queuePosition ? t('resourceDetail.queuedAtPosition', { position: queuePosition }) : t('resourceDetail.queued')) : ""}
                                            aria-label={isDownloaded && !isCorrupted ? t('resourceDetail.downloadedAriaLabel') : undefined}
                                        >
                                            {isDownloading ? (
                                                <>
                                                ...
                                                <>
                                                    ...
                                                    {t('resourceDetail.resumeProgress', { progress: progress ?? 0 })}
                                                </>
                                            ) : isPending ? (
                                                <>
                                                    <Clock className="h-5 w-5" />
                                                    {queuePosition ? t('resourceDetail.queuedAtPosition', { position: queuePosition }) : t('resourceDetail.queued')}
                                                </>
                                            ) : isCorrupted ? (
                                                <>
                                                    <RotateCcw className="h-5 w-5" />
                                                    {t('resourceDetail.retryDownload')}
                                                </>
                                            ) : isDownloaded ? (
                                                <>
                                                    <Check className="h-5 w-5" />
                                                    {t('resourceDetail.downloaded')}
                                                </>
                                            ) : (
                                                <>
                                                    <Download className="h-5 w-5" />
                                                    {t('resourceDetail.downloadNow')}
                                                </>
                                            )}
                                        </button>
                                        ...
                                        {(isDownloading || isPaused || isPending) && (
                                            <button
                                                ...
                                                title={isPending ? t('resourceDetail.removeFromQueue') : t('resourceDetail.stopAndDelete')}
                                            >
                                    ...
                                    {isCorrupted && (
                                        <div className="flex items-center gap-2 text-xs text-destructive font-semibold bg-destructive/10 p-2 rounded">
                                            ...
                                            {t('resourceDetail.corruptedWarning')}
                                        </div>
                                    )}
                                    ...
                                    {error && (
                                        <div className="w-full text-xs text-destructive font-medium bg-destructive/10 p-2 rounded animate-in fade-in slide-in-from-top-1">
                                            {error === "Work directory not configured"
                                                ? t('resourceDetail.workDirNotConfigured')
                                                : error}
                                        </div>
                                    )}
                                </div>
                    ...
                    <div className="space-y-4">
                        <div className="flex justify-between items-start">
                            <div>
                                <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground">{t('resourceDetail.category')}</span>
                            ...
                            <div className="flex flex-col items-end gap-1.5 pt-1">
                                ...
                                <div className="group relative flex items-center">
                                    ...
                                    <div className="absolute right-0 top-full mt-2 w-48 p-2 bg-popover text-popover-foreground text-xs rounded-md shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 border border-border">
                                        {t('resourceDetail.autoDownloadTooltipPrefix')}<strong>{resource.category}</strong>{t('resourceDetail.autoDownloadTooltipSuffix')}
                        ...
                        <div>
                            <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground">{t('resourceDetail.date')}</span>
                            <p className="text-base text-foreground">
                                {new Date(resource.created_at).toLocaleDateString(lang === 'it' ? 'it-IT' : 'en-US', {
                                    weekday: 'long',
                                    year: 'numeric',
                                    month: 'long',
                                    day: 'numeric'
                        ...
                        <div>
                            <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground">{t('resourceDetail.description')}</span>
                            <p className="text-base text-foreground whitespace-pre-wrap mt-1 leading-relaxed">
                                {resource.description || t('resourceDetail.noDescription')}
                            </p>
                        </div>
                    </div>
                </div>
